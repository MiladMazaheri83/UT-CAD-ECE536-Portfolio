module HashGeneratorDatapath #(
    ...
    MemoryBlock #(.WIDTH(WORD), .HEIGHT(64), .FILE_PATH("./data/constant.mem")) Rom(
        .read(romRead),
        .addr(cnt6Out),
        .dataOut(romOut)
    );
    ...
    LeftRotate #(.IDX_WIDTH(6), .WIDTH(WORD)) LeftRotate_(
        .dataIn(fOut),
        .index(cnt6Out),
        .dataOut(rotateOut)
    );
    ...
    // This multiplexer is used to choose between the leftRotated to be added with B, or  Mux2 to be added with F.
